...
class NacosClient implements NacosClientInterface
{
        ...
        do {
            ...
            try {
                ...
                if ($response->getBody()->getContents()) {
                    ...
                    ListenerConfigSuccessListener::notify($config);
                }
            } catch (Exception $e) {
                LogUtil::error("listener请求异常, e: " . $e->getMessage());
                ListenerConfigRequestErrorListener::notify($dataId, $group, $tenant);
                // 短暂休息会儿
                usleep(500);
            }
    ...
    public static function get($dataId, $group, $tenant)
    {
        ...
        try {
            ...
            LogUtil::error("从nacos服务端获取配置失败，请检查nacos配置项是否填写。错误信息：". $e->getMessage());

            LogUtil::info("尝试获取本地快照配置...");
            // 获取快照配置
            $config = LocalConfigInfoProcessor::getSnapshot($dataId, $group, $tenant);

            LogUtil::info("尝试获取本地默认配置...");
            // 获取本地默认配置
            $config = $config ? $config
                : LocalConfigInfoProcessor::getFailover($dataId, $group, $tenant);

            $configListenerParameter = Config::of($dataId, $group, $tenant, $config);
